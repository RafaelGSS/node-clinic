...
async function runTool (toolName, Tool, version, args, uiOptions) {
  ...
  function onsigint () {
    ...
    /* istanbul ignore next: isSpinning is false when spawn process. */
  ...
  if (args['collect-only']) {
    ...
    tool.collect(args['--'], function (err, filename) {
      ...
      /* istanbul ignore if: iEnabled is always false when spawn process. See: https://github.com/sindresorhus/ora#isenabled */
      if (spinner.isEnabled) {
        spinner.stop()
